...
def log_validation(
    vae, text_encoder, tokenizer, unet, controlnext, clip_vt_model,proj_model, args, accelerator, weight_dtype, step, is_final_validation=False
):
    ...
    pipeline = StableDiffusionControlNeXtPipeline.from_pretrained(
        ...
        proj_image_adapter = proj_model,
        image_encoder= clip_vt_model
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default= None,
        help= 'text_encoder model name or path')
    ...
    parser.add_argument(
        ...
        type=str,
        default="original_image column",
        help="The column of the original image .",
    )
    parser.add_argument(
        "--original_image_mask_column",
        type=str,
        default="masked image column",
        help="The column of  image have range to fill in .",
    )

    parser.add_argument(
        "--ipadapter_image_column",
        type=str,
        default="ip adapter image column",
        help="The column of  image prompt .",
    )
...
#         train_dataset = datasets.Dataset.from_dict(dict, features=datasets.Features({"image": datasets.Image(), "pose": datasets.Image(), "caption": datasets.Value(dtype='string', id=None), 'original_image':datasets.Image(),'original_image_mask': datasets.Image(),'ipadapter_image': datasets.Image()}))
...
#         original_image_column = column_names[3]
#         logger.info(f"original image column defaulting to {original_image_column}")
#     else:
...
#     if args.original_image_mask_column is None:
#         original_image_mask_column = column_names[4]
#         logger.info(f"original image column defaulting to {original_image_mask_column}")
#     else:
#         original_image_mask_column = args.original_image_mask_column
#         if original_image_mask_column not in column_names:
#             raise ValueError
#     if args.ipadapter_image_column is None:
#         ipadapter_image_column = column_names[5]
#         logger.info(f'ipadapter_image_column defaulting to {ipadapter_image_column}')
#     else:
#         ipadapter_image_column = args.ipadapter_image_column
#         if ipadapter_image_column not in column_names:
#             raise ValueError
...
#         original_images = [image_transforms(image) for image in original_images]

#         original_image_mask = [image.convert('RGB') for image in examples[original_image_mask_column]]
#         original_image_mask = [image_transforms[image] for image in original_image_mask ]

#         ipadapter_images = [image.convert("RGB") for image  in examples[ipadapter_image_column]]
#         ipadapter_images = [image_transforms[image] for image in ipadapter_images ]     

        

#         examples["pixel_values"] = images
#         examples["conditioning_pixel_values"] = conditioning_images
#         examples['original_images'] = original_images
#         examples['original_images_mask'] = original_images
#         examples["input_ids"] = tokenize_captions(examples)
#         examples['ipadapter_images'] = ipadapter_images
...
#############################################################
    text_encoder  = CLIPTextModel.from_pretrained(args.text_encoder,
    ...
    train_dataset = StableDiffusionDataset(args.dataset_path,tokenizer, 'merge' )
    ####################################################################
    ...
    controlnext,proj_model, unet, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        controlnext,proj_model, unet, optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnext, unet):
                ...
                #conditional method for training
                ...
                # Convert images to latent space
                    ...
                    original_image_mask_resize = torch.nn.functional.interpolate(original_image_mask, size = (latents.shape[2], latents.shape[3]))
                    ...
                    noisy_latents= noisy_latent_targets

                    controlnext_image = batch["original_values"].to(dtype=weight_dtype)
                    controlnext_output = controlnext(controlnext_image, timesteps)
